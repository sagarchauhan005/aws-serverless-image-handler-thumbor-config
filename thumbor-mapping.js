...
class ThumborMapping {
    ...
    process(event) {
        ...
        this.consoleLog("Filetype is : "+filetype);
        // Process the Dimensions
        const dimPath = this.path.match(/\/((\d+x\d+)|(0x\d+))\//g); //100x100
        this.consoleLog("Dimensions are : "+dimPath);
        if (dimPath) {
            ...
            const width = Number(dims[0]); //100
            const height = Number(dims[1]); //100
            ...
            if (!isNaN(width) && !isNaN(height)) {
                ...
                this.edits.resize.width = width === 0 ? 1920 : width;  //default width
                this.edits.resize.height = height === 0 ? 1080 : height; //default height
            }
        }

        // fit-in filter
        if (
            this.path.includes('fit-in') ||
            this.path.includes('cover') ||
            this.path.includes('inside') ||
            this.path.includes('outside') ||
